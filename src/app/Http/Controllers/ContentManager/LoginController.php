...
class LoginController extends Controller
{
    ...
    {
        $title = 'Login';
        return view('content-manager.pages.login', compact('title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if(Auth::guard('content_manager')->attempt($credentials)){
            return redirect()->route('cm-dashboard', BergyUtils::uuid())
    ...
    public function logout(Request $request) {
        session()->flush();
        Auth::guard('content_manager')->logout();
        return redirect()->to('/content-manager/login')
